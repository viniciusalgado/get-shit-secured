...
try {
  ...
  const rolloutMode = runtimeManifest.rolloutMode || 'mcp-only';
  if (mcpReady && corpusReady) {
    console.log('[GSS] v' + gssVersion + ' | corpus v' + corpusVersion + ' | ' + docCount + ' docs | MCP ready | ' + workflowCount + ' workflows | ' + roleCount + ' roles | ' + mcpName + ' | mode: ' + rolloutMode + ' | installed ' + runtimeManifest.installedAt.split('T')[0]);
  } else {
...
try {
  ...
  if (validator && typeof validator.validateArtifact === 'function') {
    const result = validator.validateArtifact(artifact, workflowId, { artifactPath: normalizedPath });
...
function generateArtifactValidatorModule(): string {
  const rulesEntries = Object.entries(ARTIFACT_VALIDATION_RULES).map(([id, rules]) =>
    `  '${id}': ${JSON.stringify(rules)}`
  ).join(',\n');

  return `// GSS Artifact Validator — installed support module
// Generated by get-shit-secured Phase 14
...
function validateArtifactEnvelope(artifact, expectedWorkflowId) {
  var errors = [];
  if (typeof artifact !== 'object' || artifact === null || Array.isArray(artifact)) {
    errors.push('Artifact must be a non-null object');
    return { valid: false, errors: errors };
  }
  if (typeof artifact.schemaVersion !== 'number') errors.push('schemaVersion must be a number');
  if (typeof artifact.workflowId !== 'string') errors.push('workflowId must be a string');
  if (typeof artifact.gssVersion !== 'string') errors.push('gssVersion must be a string');
  if (typeof artifact.corpusVersion !== 'string') errors.push('corpusVersion must be a string');
  if (typeof artifact.generatedAt !== 'string') errors.push('generatedAt must be a string');
  if (typeof artifact.consultationMode !== 'string') errors.push('consultationMode must be a string');
  if (expectedWorkflowId && artifact.workflowId !== expectedWorkflowId) {
    errors.push('workflowId must match expected workflow');
  }
  return { valid: errors.length === 0, errors: errors };
}

function requireArray(artifact, field) {
  if (!(field in artifact)) return field + ' is missing';
  if (!Array.isArray(artifact[field])) return field + ' must be an array';
  return null;
}

function getCoverageStatus(artifact) {
  var consultation = artifact && artifact.consultation;
  if (!consultation || typeof consultation !== 'object') return 'not-applicable';
  if (consultation.validation && typeof consultation.validation === 'object') {
    var nested = consultation.validation.coverageStatus;
    if (nested === 'pass' || nested === 'warn' || nested === 'fail') return nested;
  }
  var flat = consultation.coverageStatus;
  if (flat === 'pass' || flat === 'warn' || flat === 'fail') return flat;
  return 'missing';
}

function findRule(workflowId, artifactPath, artifactName) {
  var rules = ARTIFACT_VALIDATION_RULES[workflowId] || [];
  if (artifactName) {
    for (var i = 0; i < rules.length; i++) {
      if (rules[i].artifactName === artifactName) return rules[i];
    }
  }
  if (artifactPath) {
    var normalized = String(artifactPath).replace(/\\\\/g, '/');
    for (var j = 0; j < rules.length; j++) {
      if (normalized.endsWith(rules[j].path)) return rules[j];
    }
  }
  return rules[0] || null;
}

function validateArtifactByName(artifact, workflowId, artifactName, requiresConsultationTrace) {
  var envelope = validateArtifactEnvelope(artifact, workflowId);
  if (!envelope.valid) return envelope;

  var errors = [];
  if (requiresConsultationTrace) {
    if (!artifact.consultation || typeof artifact.consultation !== 'object') {
      errors.push('consultationMode is "required" but "consultation" section is missing');
    }
  }

  var arrayFieldByArtifact = {
    'security-review::change-scope': 'changedFiles',
    'security-review::findings': 'findings',
    'map-codebase::codebase-inventory': 'components',
    'map-codebase::trust-boundary-map': 'boundaries',
    'map-codebase::dependency-map': 'dependencies',
    'map-codebase::data-flow-map': 'flows',
    'threat-model::threat-register': 'threats',
    'threat-model::risk-assessment': 'risks',
    'audit::findings-report': 'findings',
    'audit::remediation-priorities': 'priorities',
    'validate-findings::validation-report': 'findings',
    'validate-findings::validated-findings': 'findings',
    'validate-findings::tdd-test-document': 'testSpecs',
    'plan-remediation::patch-plan': 'patches',
    'plan-remediation::test-specifications': 'tests',
    'execute-remediation::application-report': 'applied',
    'verify::verification-report': 'verifications'
  };
  var arrayField = arrayFieldByArtifact[workflowId + '::' + artifactName];
  if (arrayField) {
    var arrErr = requireArray(artifact, arrayField);
...
function validateArtifact(artifact, workflowId, options) {
  var rule = findRule(workflowId, options && options.artifactPath, options && options.artifactName);
  if (!rule) {
    return {
      valid: true,
      errors: [],
  ...
  var result = validateArtifactByName(artifact, workflowId, rule.artifactName, rule.requiresConsultationTrace);
  return {
    valid: result.valid,
